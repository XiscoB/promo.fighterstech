...
- **Build Process**: `npm run build` (node build.js) compiles `template.html` + `lang/*.json` into `dist/` (EN at root, ES at `/es/`). `dist/` is a gitignored build artifact - never edit it directly
- **Package Management**: `package.json` with `sharp` (image tooling); the site itself has no runtime dependencies
- **Hosting**: GitHub Pages with custom domain (promo.fighterstech.com), deployed by `.github/workflows/deploy.yml` on every push/merge to `main`
...
├── dist/                   # Build output (gitignored, regenerated by CI)
├── kickstarter/
...
1. **Loading**: The built pages fetch the appropriate JSON file from `/lang/` based on user language (`navigator.language`)
...
4. Add the language option to the language toggle in `template.html` and a matching `buildHtml` call in `build.js`
...
1. **HTML**: Edit `template.html` - it is the single source of truth for the landing page. Never edit files in `dist/` (build output) and never create a loose root `index.html` (that divergence caused a deploy incident)
...
3. Add new `<div class="swiper-slide">` elements to the app-images section in `template.html`
...
This site is deployed via **GitHub Pages** using `.github/workflows/deploy.yml`:

1. Push or merge **any branch into `main`** - the workflow triggers automatically
2. CI runs `npm install && npm run build` and deploys the freshly built `dist/` as a Pages artifact
...
**Changes are live ~1-2 minutes after the workflow finishes** (green run in the Actions tab). GitHub's CDN caches pages for 10 minutes, so hard-refresh (Ctrl+Shift+R) if you don't see changes.

### Local Development

To preview locally, build first and serve the output:

```bash
npm run build
python -m http.server 8000 --directory dist
# Or: npx serve dist -l 8000
```

Then open `http://localhost:8000` (EN) and `http://localhost:8000/es/` (ES) in your browser. This lets you test everything without committing.
